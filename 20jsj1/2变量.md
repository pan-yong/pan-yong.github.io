...
<img src=".\images\floatdoublechar.png" alt="img" style="zoom:80%;" />

不同的数据类型变量，相当于不同类型的盒子，存放不同类型的数据。`int` 表示整型，对应盒子只能存放整型数据。`float` 表示浮点数，对应盒子只能存放小数。`double`双精度浮点数，存储比`float`精度更高的小数。`char` 字符类型，存储单引号括起来的单个字符。
...
格式串中的 `%` 是控制符：`%d` 整数，`%f` 浮点数，`%c` 字符。格式串中非控制符内容会保持原样的输出。
...
我们说变量就像一个盒子用来放数据，实际我们程序运行时数据在内存上，那变量在内存的哪个位置呢？通过取地址运算 `&a` ，可以得到变量的内存地址。这个内存地址很重要，知道变量的地址才能通过键盘把数据送入到变量对应内存中。
