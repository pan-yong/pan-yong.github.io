...
`int` 表示整型，对应盒子只能存放整型数据。`float` 表示浮点型，对应盒子只能存放小数。`double`表示双精度浮点型，能存储比`float`精度更高的小数。`char` 字符类型，存储单引号括起来的单个字符。
...
不同类型的变量，相当于不同类型的盒子，存放不同类型的数据：

<img src=".\images\type.jpg" alt="img" style="zoom:40%;" />

不同类型的变量，盒子大小也不一样，存放的数据范围也不一样：
