...
变量就像一个盒子用来存放数据。C语言中，`int` 表示整型，对应盒子只能存放整型数据。`float` 表示浮点型，对应盒子只能存放小数，`double`表示双精度浮点型，能存储比`float`精度更高的小数。`char` 字符类型，用来存储单引号括起来的单个字符。
...
用 `printf` 函数输出变量的值。`printf`第一个参数是用双引号括起来的一串字符，称为格式串，用来控制输出，双引号中有什么字符就会输出什么字符，格式串中的 `%` 是控制符：`%d` 整数，`%f` 浮点数，`%c` 字符。
