...
​	注意：也只有在定义数组时才可以同时给数组所有元素赋。定义过后只能通过下标一个一个访问赋值。
...
int main()
{
    ...
    for(i = 0; i < N; i)
        scanf("%f", ___________);  // scores +i 或者  &scores[i]
    
    for(sum = 0, i = 0; i < N; i++)
        __________________;		// 求总分
    ave = sum / N;				// 求平均分
...
int main()
{
    ...
    do
    {
        ____________;	//b[i++] = n%2
        n = n / 2;
    }while(n);
    for(; i > 0; i--)
        printf("%d",______);	//b[i-1]
    return 0;
}
```
